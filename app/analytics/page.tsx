          ...
          {data.map((d) => {
            ...
            return (
              <div key={`${d.y}-${d.m}`} className="space-y-1">
                <div className="text-sm flex justify-between"><span>{d.y}-{String(d.m).padStart(2, '0')}</span><strong>{d.revenue.toFixed(2)}</strong></div>
        ...
        <section className="border erp-border rounded p-3 space-y-2">
          ...
          <div className="overflow-auto border erp-border rounded">
            <table className="w-full text-sm">
              ...
              <tbody>{data.map((d) => (<tr key={`o-${d.y}-${d.m}`} className="[&>td]:px-2 [&>td]:py-1 border-b erp-border"><td>{d.y}-{String(d.m).padStart(2, '0')}</td><td>{d.overdue}</td></tr>))}</tbody>
            </table>
          </div>
        </section>
      )}
        ...
        <section className="border erp-border rounded p-3 space-y-2">
          ...
          <div className="overflow-auto border erp-border rounded">
            <table className="w-full text-sm">
              ...
              <tbody>{data.map((d) => (<tr key={`c-${d.y}-${d.m}`} className="[&>td]:px-2 [&>td]:py-1 border-b erp-border"><td>{d.y}-{String(d.m).padStart(2, '0')}</td><td>{clampPercent(d.collectionRate).toFixed(1)}%</td><td>{d.avgCollectionDays.toFixed(1)} วัน</td></tr>))}</tbody>
            </table>
          </div>
        </section>
      )}
