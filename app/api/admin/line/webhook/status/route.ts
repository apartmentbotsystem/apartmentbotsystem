...
export async function GET(req: Request) {
  try {
    ...
    if (tokenOk) {
      try {
        info = await withTimeout(10_000, () => fetchWebhookInfo(token))
      } catch {
        info = null
      }
    }
...
export async function POST(req: Request) {
  try {
    const user = await requireSession(req)
    enforceRoleBoundary(user, ['OWNER', 'ADMIN', 'STAFF'])
    const token = await withTimeout(10_000, () => getLineAccessTokenPreferDb())
    const res = await withTimeout(10_000, () => fetch('https://api.line.me/v2/bot/channel/webhook/test', {
      method: 'POST',
      headers: {
