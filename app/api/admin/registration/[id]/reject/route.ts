...
import { rejectRegistration } from '@/services/registrationReview.service'
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const rl = checkRateLimitCustom(`${getClientIp(req)}:${user.id}:/api/admin/registration/[id]/reject:POST`, 5000, 10)
    ...
    const result = await withTimeout(10_000, () => rejectRegistration(params.id, parse.data.reason))
