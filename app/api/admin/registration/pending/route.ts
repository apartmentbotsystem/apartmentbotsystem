...
export async function GET(req: Request) {
  try {
    const rl = checkRateLimit(getClientIp(req), '/api/admin/registration/pending:GET')
    ...
    const items = await withTimeout(10_000, () =>
      runWithRequestContext({ route: '/api/admin/registration/pending:GET', userId: user.id }, () =>
        prisma.registrationRequest.findMany({
          where: { status: 'PENDING' },
          orderBy: { createdAt: 'desc' },
          select: { id: true, lineUserId: true, roomNumber: true, residentName: true, phone: true, createdAt: true },
          take: limit
        })
      )
    )
