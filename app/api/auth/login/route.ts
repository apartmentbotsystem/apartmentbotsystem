...
export async function POST(req: Request) {
  try {
    const rl = checkRateLimitCustom(`${getClientIp(req)}:/api/auth/login:POST`, 60_000, 10)
    if (!rl.allowed) return NextResponse.json({ error: 'RATE_LIMIT', message: 'Too many attempts' }, { status: 429 })
    const body = await req.json()
    const parse = schema.safeParse(body)
    if (!parse.success) return NextResponse.json({ error: 'UNPROCESSABLE', message: 'Invalid body' }, { status: 422 })

    const e2eBypassEnabled = process.env['E2E_ALLOW_ANY_USER'] === 'true' || process.env['NODE_ENV'] !== 'production'
    ...
    if (!userId && parse.data.email) {
      const user = await withTimeout(10_000, () => prisma.user.findUnique({
    ...
    const token = await withTimeout(10_000, () => createSession(userId, role, sessionVersion))
