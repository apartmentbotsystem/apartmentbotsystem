...
export async function POST(req: Request) {
  try {
    ...
    if (!verifyCsrf(req)) return NextResponse.json({ ok: false, code: 'CSRF', message: 'Invalid CSRF token' }, { status: 403 })
    const rl = checkRateLimitCustom(`${getClientIp(req)}:${user.id}:/api/billing/import:POST`, 5000, 10)
    ...
    const result = await withTimeout(10_000, () => ImportSvc.runImport(user, { year, month, fileBuf: buf, strict, headerMapping }))

    if (!result.ok) {
      const status = (result.code === 'MONTH_ALREADY_IMPORTED' || result.code === 'IMPORT_EXISTS_PROCESSING') ? 409 : 400
...
> {
  const body = await withTimeout(10_000, () => req.json()) as {
    fileBase64?: string
    year?: unknown
    month?: unknown
    strict?: unknown
...
async function parseFormMeta(req: Request): Promise<
  ...
  | { ok: false; message: string }
> {
  const form = await withTimeout(10_000, () => req.formData())
  ...
  return { ok: true, file: Buffer.from(await withTimeout(10_000, () => file.arrayBuffer())), data: parse.data }
}
