...
export async function GET(req: Request) {
  try {
    ...
    const items = await withTimeout(10_000, () =>
      runWithRequestContext({ route: '/api/billing/months:GET', userId: user.id }, () =>
        BillingMonths.listMonthsSummary(user)
      )
    )
    return NextResponse.json({ items: Array.isArray(items) ? items.slice(0, limit) : items })
  } catch (err) {
