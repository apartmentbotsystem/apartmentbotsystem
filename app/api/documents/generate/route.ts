...
export async function POST(req: Request) {
  try {
    ...
    const rl = checkRateLimitCustom(`${getClientIp(req)}:${user.id}:/api/documents/generate:POST`, 5000, 10)
    ...
    const { reused, result } = await withTimeout(10_000, () => ensureIdempotent('/api/documents/generate', idemKey, payloadHash, async () => {
      return Documents.generateDocument(user, templateId, roomNumber, year, month, force, user.id)
    }))
    logInfo('documents.generate', { userId: user.id, templateId, roomNumber, year, month, reused })
