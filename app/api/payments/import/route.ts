...
export async function POST(req: Request) {
  try {
    ...
    if (!(file instanceof Blob)) return NextResponse.json({ error: 'UNPROCESSABLE', message: 'กรุณาแนบไฟล์' }, { status: 422 })
    ...
    if (!ws) return NextResponse.json({ error: 'UNPROCESSABLE', message: 'ไม่พบชีต' }, { status: 422 })
