...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const t = await withTimeout(10_000, () => Templates.getTemplateMeta(user, params.id))
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const updated = await withTimeout(10_000, () => Templates.renameTemplate(user, params.id, parse.data.name))
