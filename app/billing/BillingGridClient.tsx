...
import type { CellFocusedEvent, CellValueChangedEvent, ColDef, GridReadyEvent } from 'ag-grid-community'
...
function toSheetRows(rows: BillingEditorRow[]): SheetRow[] {
  return rows.map((r) => {
    const row: SheetRow = { id: r.id, roomNumber: r.roomNumber }
...
const GRID_FIELD_ORDER = Array.from({ length: REQUIRED_BILLING_HEADERS.length }, (_, i) => hKey(i))
...
export default function BillingGridClient({ year, month, locked, initialRows }: Props) {
  ...
  const ROWNUM_COL_PX = 56
  const ROOM_COL_MIN_PX = 110
  ...
  const autoSizeInputEl = (el: HTMLInputElement | null) => {
    if (!el) return
    const cs = window.getComputedStyle(el)
    const ctx = getMeasureCtx()
    if (!ctx) return
    const font =
      (cs as any).font && (cs as any).font !== 'inherit'
        ? (cs as any).font
    ...
    const extra = 10
    const minPx = 56
    el.style.width = `${Math.max(minPx, textWidth + paddings + extra)}px`
  }
  ...
  const applyEditById = (rowId: string, field: string, value: unknown) => {
    if (locked || !isEditableField(field)) return
    setRows((prev) => {
      const idx = prev.findIndex((r) => r.id === rowId)
      if (idx === -1) return prev
      const target = prev[idx]
      let next = { ...target }
      const i = field.startsWith('h') ? Number(field.slice(1)) : -1
      if (i < 0 || i === H.room || CALC_INDEXES.has(i)) return prev
      const isNumeric = i >= 5 && i <= 20
      const normalized = isNumeric ? normalizeNumericTyping(String(value ?? '')) : String(value ?? '')
      next = calcRow({ ...next, [field]: normalized })
  ...
  const totals = useMemo(() => {
    return rows.reduce(
      ...
      { rent: 0, water: 0, electric: 0, other: 0, amount: 0 }
    )
  }, [rows])
  ...
  const pinnedBottomRowData = useMemo<SheetRow[]>(() => {
    const row: SheetRow = { id: '__total__', roomNumber: '' }
  ...
  return (
    <div className="space-y-2">
      ...
      >
        {viewMode === 'simple' ? (
          <div className="overflow-auto" style={{ height: '72vh', width: '100%' }}>
            <table className="min-w-[1400px] w-full text-xs border-collapse">
              <thead className="sticky top-0 bg-white">
                <tr>
                  <th
                    className="text-left px-2 py-1 border erp-border whitespace-nowrap bg-white"
                    style={{ position: 'sticky', left: 0, zIndex: 2, width: ROWNUM_COL_PX, minWidth: ROWNUM_COL_PX }}
                  >
                    #
                  </th>
                  {REQUIRED_BILLING_HEADERS.map((h, idx) => {
                    const isRoom = idx === H.room
                    return (
                      <th
                        key={h}
                        className={`text-left px-2 py-1 border erp-border whitespace-nowrap bg-white ${isRoom ? 'z-[2]' : ''}`}
                        style={
                          isRoom
                            ? { position: 'sticky', left: ROWNUM_COL_PX, zIndex: 2, minWidth: ROOM_COL_MIN_PX }
                            : undefined
                        }
                      >
                        {h}
              ...
              <tbody>
                {rows.map((r, i) => (
                  <tr key={r.id} className="odd:bg-white even:bg-[rgba(0,0,0,0.02)]">
                    <td
                      className="px-2 py-1 border erp-border whitespace-nowrap bg-white"
                      style={{ position: 'sticky', left: 0, zIndex: 1, width: ROWNUM_COL_PX, minWidth: ROWNUM_COL_PX }}
                    >
                      {i + 1}
                    </td>
                    {REQUIRED_BILLING_HEADERS.map((_, idx) => {
                      const key = hKey(idx)
                      const isRoom = idx === H.room
                      const isCalc = CALC_INDEXES.has(idx)
                      const val = String((r as Record<string, unknown>)[key] ?? '')
                      if (isRoom || isCalc) {
                        return (
                          <td
                            key={`${r.id}-${idx}`}
                            ...
                            style={
                              isRoom
                                ? { position: 'sticky', left: ROWNUM_COL_PX, zIndex: 1, minWidth: ROOM_COL_MIN_PX }
                                : undefined
                            }
                          >
                            {val}
                      ...
                      const isNumeric = idx >= 5 && idx <= 20
                      return (
                        <td key={`${r.id}-${idx}`} className="px-1 py-0.5 border erp-border">
                          <input
                            value={val}
                            onChange={(e) => applyEditById(r.id, key, e.target.value)}
                            className={`px-2 py-1 border rounded ${isNumeric ? 'text-right' : ''}`}
                            ref={(el) => autoSizeInputEl(el)}
                            onInput={(e) => autoSizeInputEl(e.currentTarget)}
                            style={{ minWidth: 56 }}
              ...
              <tfoot>
                <tr className="font-semibold bg-[var(--bg-surface)]">
                  <td className="px-2 py-1 border erp-border">รวม</td>
                  {REQUIRED_BILLING_HEADERS.map((_, idx) => {
                    let v = ''
                    if (idx === H.rent) v = formatMoney(totals.rent)
                    if (idx === H.waterTotal) v = formatMoney(totals.water)
                    if (idx === H.elecTotal) v = formatMoney(totals.electric)
                    if (idx === H.other) v = formatMoney(totals.other)
                    if (idx === H.grand) v = formatMoney(totals.amount)
                    return <td key={`t-${idx}`} className={`px-2 py-1 border erp-border ${v ? 'text-right' : ''}`}>{v}</td>
                  })}
                </tr>
              </tfoot>
            </table>
          ...
          <AgGridReact<SheetRow>
            ...
            onGridReady={(e: GridReadyEvent<SheetRow>) => {
              try {
