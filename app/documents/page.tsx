...
export default async function DocumentsPage({ searchParams }: { searchParams?: Params & { page?: string } }) {
  ...
  const hasNext = skip + docs.length < totalCount
  ...
  return (
    <div className="space-y-3">
            ...
            {docs.map((doc) => {
              ...
              return (
                <tr key={doc.id} className="[&>td]:px-2 [&>td]:py-1 border-b erp-border align-top">
                  <td>{toDisplayZoned(doc.generatedAt)}</td>
      ...
      <div className="flex items-center justify-between text-sm mt-2">
        <div>หน้า {page} / {Math.max(1, Math.ceil(totalCount / take))} • ทั้งหมด {totalCount} รายการ</div>
        <div className="flex gap-2">
          <Link
            href={`/documents?${new URLSearchParams({ ...(Number.isFinite(floorIdx) ? { floor: String(floorIdx) } : {}), ...(status ? { status } : {}), ...(Number.isFinite(versionNo) ? { version: String(versionNo) } : {}), page: String(Math.max(1, page - 1)) }).toString()}`}
            className={`px-2 py-1 border erp-border rounded ${hasPrev ? '' : 'opacity-50 pointer-events-none'}`}
            aria-label="หน้าก่อนหน้า"
          >
            ก่อนหน้า
          </Link>
          <Link
            href={`/documents?${new URLSearchParams({ ...(Number.isFinite(floorIdx) ? { floor: String(floorIdx) } : {}), ...(status ? { status } : {}), ...(Number.isFinite(versionNo) ? { version: String(versionNo) } : {}), page: String(page + 1) }).toString()}`}
            className={`px-2 py-1 border erp-border rounded ${hasNext ? '' : 'opacity-50 pointer-events-none'}`}
