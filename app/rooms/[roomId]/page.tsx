...
export default async function RoomDetailPage({ params }: { params: { roomId: string } }) {
  ...
  const usageSeries = usageRows
    .map((row) => {
      ...
      return {
        ym: `${row.billingMonth.year}-${String(row.billingMonth.month).padStart(2, '0')}`,
        water: usage.water,
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="text-xs opacity-70">Billing Month: {year}-{String(month).padStart(2, '0')} | Consumption Period: {consumptionYm}</div>
        </div>
            ...
            {versions.map((v) => {
              ...
              return (
                <tr key={v.id} className="border-t erp-border">
                  <td className="px-2 py-1">{bm ? `${bm.year}-${String(bm.month).padStart(2, '0')}` : '-'}</td>
          ...
          <tbody>
            {docs.map((doc) => (
              <tr key={doc.id} className="border-t erp-border">
                ...
                <td className="px-2 py-1">{doc.billingMonth ? `${doc.billingMonth.year}-${String(doc.billingMonth.month).padStart(2, '0')}` : '-'}</td>
