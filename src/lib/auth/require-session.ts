...
import { mapRoleToLegacy } from '@/lib/auth/session'
...
export async function requireSession(req: Request): Promise<AuthUser> {
  ...
  try {
    ...
    if (Array.isArray(rows) && rows.length > 0) {
      const dbLegacyRoles = rows.map(r => mapRoleToLegacy(r.code))
      const tokenLegacy = mapRoleToLegacy(payload.role)
      if (!dbLegacyRoles.includes(tokenLegacy)) {
  ...
  const legacyRole = mapRoleToLegacy(payload.role)
  return { id: payload.sub, role: legacyRole }
}

export function enforceRoleBoundary(user: AuthUser, allowedRoles: Array<'SUPER_ADMIN' | 'FINANCE' | 'STAFF' | 'VIEWER' | 'ADMIN' | 'MANAGER' | 'ACCOUNTANT'>): void {
  const allowLegacy = allowedRoles.map((r) => mapRoleToLegacy(r))
  const effective = mapRoleToLegacy(user.role)
  if (!allowLegacy.includes(effective)) {
