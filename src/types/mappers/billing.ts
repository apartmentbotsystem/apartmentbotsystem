...
export function mapBillingMonthsDbToDomain(items: Array<Db.BillingMonth & { records: Array<{ amount: unknown; adjustments: unknown; payments: Array<{ matchedAmount: unknown }> }> }>): Domain.BillingMonthSummary[] {
  return items.map((m) => {
    const totalBilled = m.records.reduce((s, r) => s + toNumberSafe(r.amount) + toNumberSafe(r.adjustments), 0)
...
export function mapBillingRecordsApiToDomain(api: { items: Array<{ id: string; roomNumber: string; amount: number; adjustments: number; note: string; dueDate: string | null; overdueDays: number; penalty: number; status: string }> }): Domain.BillingRecord[] {
